...
JOBS_QUERY = f"""
SELECT
  J.HLINK_IDX            AS BOARD_IDX,
  J.TITLE               AS TITLE,
  J.DISPLAY_TITLE       AS DISPLAY_TITLE,
  J.CONTENT             AS CONTENT,
  J.ORG_NAME            AS ORGANIZATION_NAME,
  J.START_DATE          AS START_DATE,
  J.END_DATE            AS END_DATE,
  J.REG_DATE            AS REG_DATE,
  J.PAY                 AS PAY_RAW,
  J.PAY_TYPE            AS PAY_TYPE,
  J.INCENTIVE_FLAG      AS INCENTIVE_FLAG,
  J.SPC_CODE            AS SPC_CODE,
  J.SPC_DETAIL_CODE     AS SPC_DETAIL_CODE,
  -- SPECIALTIES: 코드 두 개를 단순 병합(코드명 매핑은 별도 단계에서 처리 가능)
  TRIM(BOTH ', ' FROM CONCAT_WS(', ', NULLIF(J.SPC_CODE, ''), NULLIF(J.SPC_DETAIL_CODE, ''))) AS SPECIALTIES,
  J.RECRUIT_FLAG,
  J.DISPLAY_FLAG,
  J.DEL_FLAG
...
WHERE J.START_DATE >= '{DATE_START}'
  ...
  AND J.RECRUIT_FLAG = 'Y'
  AND J.DISPLAY_FLAG = 'Y'
  AND (J.DEL_FLAG IS NULL OR J.DEL_FLAG <> 'Y');
"""
